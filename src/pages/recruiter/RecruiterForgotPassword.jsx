...
import { recuiterForgotPassword, recuiterVerifyOtp, recuiterResetPassword } from "../../api/api";
...
const EnterPasswordFunction = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (userData.new_password !== userData.confirm_password) {
      ...
      setLoading(true)
    await recuiterResetPassword(userData)
      .then((res) => {
        setLoading(false)
        const { response } = res;
...
const OtpFunction = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true)
    await recuiterVerifyOtp(userData)
      .then((res) => {
        setLoading(false)
        const { response } = res;
        if (response?.data.error === true) {
...
function RecruiterForgotPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    await recuiterForgotPassword(userData)
      .then((res) => {
        const { response } = res;
        setLoading(false)
        if (response?.data.error === true) {
